...
export default function RegisterPage() {
  ...
  const createDefaultBranch = async (userId: string) => {
    const branchesCollectionRef = collection(db, 'users', userId, 'branches');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await createDefaultBranch(userCredential.user.uid);
      toast({ title: 'نجاح', description: 'تم إنشاء الحساب وتسجيل الدخول بنجاح!' });
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      // Check if user is new, if so create default branch
      if (userCredential.user) {
        // This is a simplification. In a real app, you'd check if the user is truly new.
        // For now, we assume if they use this button on this page, they might need a branch.
        // A better approach is checking for existing 'branches' collection in a backend function.
        await createDefaultBranch(userCredential.user.uid);
      }
